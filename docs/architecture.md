...
account and has a 720-second deadline. It prepares one real sealed three-ticket
...
Ticket lifecycle consumers share the pure parser in
`scripts/lib/ticket_state_transition.py`: committed operator actions, trusted
ticket-state mutation, readiness, state-machine resolution, and passport
export or migration require exactly one known `State` value. Operator actions
perform the same validation before creating lock or receipt residue and again
under the action lock, so a rejected ticket cannot exploit a different
first-match parser or a validation race.
...
If protected main advances after that Ready branch is pushed, retry accepts only
...
retention keeps the immutable failed terminal and accepts both the fixed-role
v1 record and the role-explicit v2 model-identity record only for the exact
typed exit authorized by that correction; its progress identity comes from the
same signed record. Missing, changed, unsafe, ambiguous, uncorrected, or
mismatched evidence blocks before a provider starts.
...
lineage. Adjacent passport snapshots may omit ordinary descendant ticket commits,
but their Factory, protected-base, and route bindings remain exact:
