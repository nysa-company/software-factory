...
- During a concurrent PR-ready wave, defer unrelated changes to shared Factory control inputs until the cohort drains. Keep generated indexes untracked. Do not treat `factory/rulings.md` as inert: Planner and Spec-linter consume it, so a ruling change remains a semantic refresh.
- Notice: after the ticket PR merges, the dispatcher opens `chore/tNNN-closeout` from current `origin/main` and invokes `ticket-attest --action done`. It verifies the merge and configured post-merge contexts, projects the ledger once, commits Done plus closeout evidence, creates or reuses the exact factory-owned closeout PR, and requests protected auto-merge. No operator approval or manual GitHub merge is required. After protected main contains valid Done, sequencing returns `COMPLETE` and the dispatcher releases the lease. Projection automatically seals only an exact historical no-GO/no-submission `resolved` orphan as zero-cost `launch_void`; any live claim, PID, provider output, progress, process identity, sidecar, GO/submission marker, or other unsettled state still refuses.
...
repair helper may supersede a failed one while live identity exactly matches the
pre-upgrade plan and the signed cursor has not advanced beyond
`provider_cli_ready`. At `environment_upgraded` or `doctor_ready`, supersession
also requires the exact one-generation target activation and the sealed target's
canonical runtime-ledger projection. Every other post-mutation state and
completion refuse a changed helper. Missing, unsealed, and same-release helpers
also refuse. Initial repair and supersession require an existing authenticated
in-progress journal; exact completion replay remains supported after cleanup.
