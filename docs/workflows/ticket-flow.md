...
4. **Spec-linter** uses its pinned checking-lane route, remains in Planning, checks criteria quality, material contract coverage, consistency, and trust-boundary edge coverage, and appends all findings plus one `SPEC-LINT: PASS`/`FAIL` verdict line. Non-blocking exhaustive-coverage recommendations use `SPEC-WARN:` and may accompany PASS; they proceed to Test-author without a Planner round. FAIL is reserved for a missing material decision, contradiction, weakened trust boundary, data-loss risk, or unintended external effect. FAIL sends the ticket back to the planner (one replan); a second FAIL escalates to the operator. The operator may authorize only the next semantic lint round with the exact ticket line `OPERATOR AUTHORIZATION: spec-linter round <N>`; the dispatcher never writes it on its own.
