...
2. **Dispatcher** creates the exact clean ticket branch/worktree from current protected `origin/main`, then runs trusted operator-field materialization to create and verify its remote ref. It invokes sealed `models pin`, which resolves the active profile (or default `legacy-balanced-v1`) only if one ordered portfolio has ready candidates for all six roles with distinct production/checking families. The one pushed commit contains both `Kit-SHA` and `factory/route-plans/<ticket>.json`; preflight follows.
...
7. **CI** runs: lint, typecheck, tests, build, self-referential snapshots, test-immutability check.
8. **Reviewer** uses its pinned checking-lane route in Review and checks test adequacy and spec conformance. Approve, or request changes back to Building (max 2 rounds → Blocked-Escalated with a plain-language note).
9. **Narrator** uses its pinned production-lane route, remains in Review, and commits the bundle from the PR's preview deploy: plain-language summary, preview link, screenshots, criteria table, risk line, cost, rollback note. Under contract 1.3, the dispatcher then invokes trusted `ticket-attest --action bundle`; it verifies the exact reviewed SHA, every successful run's tuple against the committed six-role route plan, the route-plan blob/digest and policy hash, allowed post-review paths, bundle blob, and unique exact PR before committing Awaiting Approval. Contract 1.2 still stops in Review.
