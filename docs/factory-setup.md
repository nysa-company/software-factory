...
- Ignore `factory/runs/`, `factory/.active-runs/`, and `factory/runtime-ledger.csv`; preflight or the first normal Linear reconciliation durably initializes the real, no-follow runs root, manifests and their ownership records are atomic local run truth, claims are runtime exclusion state, and the CSV is their rebuildable view over the tracked durable ledger.
