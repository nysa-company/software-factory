...
- Ignore `factory/operator-map.json`, `factory/.operator-map.lock`, `factory/.envelope.lock/`, `factory/envelope-overrides/`, and `factory/envelope-override-consumptions/`; they are runtime operator state and must never dirty the registered checkout. Release setup verifies the operator map and its lock are ignored before it creates any product runtime state.
