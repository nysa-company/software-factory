...
`cursor-plan` binds the one-use approval to the lane nonce, factory and product trees, route plan, Cursor version, resolved executable path, and executable bytes. Cursor's `status` and `models` subcommands only inspect browser-login state, so API-key mode validates the CLI contract and defers credential/model validation to the first model-bound call, which fails closed before advancement. Cursor's hardcoded `/tmp/.cursor` scratch path is redirected through an ephemeral symlink into the lane; Seatbelt allows only that exact lexical bridge and its lane-local target, and the wrapper removes it on every sandbox exit. `cursor-run` consumes the approval before provider execution and stops on drift. The key is passed through standard input and is not written into lane state. The reviewer must stay read-only and report `APPROVE`; the final state remains `AWAIT-OPERATOR`.
