...
Seeded retries require an owner-only, single-link accounting manifest bound to the exact seed-bundle digest and approved base SHA. Its full historical ticket map is retained even when only a subset resumes. A seed preserves a supported `Ready` or `Review` stage and its prerequisite spec-lint evidence while discarding a stale Reviewer verdict, so completed roles are not needlessly rerun. V2 keeps the default $100 ticket and $500 aggregate ceilings; an explicit operator-authored v3 record permits only the bounded $200 ticket and $700 aggregate development-proof ceilings. V4 permits an operator-authored per-ticket cap map up to $300 per ticket with an exact $1,000 aggregate ceiling. V3 and V4 also bind one authorization nonce and UTC budget day; planning consumes that nonce in the owner-only artifact directory before creating a runnable lane, and day drift stops before reservation. Prior reservations reduce both limits, and the resulting per-ticket envelope remains the coordinator's atomic admission cap. Missing history, reuse, an exhausted selected ticket, aggregate exhaustion, unauthorized limits, bundle/base drift, duplicate tickets, seeded symlinks or submodules, and unsafe ticket files fail before provider execution.
