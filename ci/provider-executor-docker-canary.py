...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="provider-docker-canary.") as temporary:
        ...
        if len(results) != 6 or any(
            not item["admitted"]
            or item["execution"]["mode"] != "isolated-v1"
            or item["execution"]["return_code"] != 0
            for item in results
        ):
            raise RuntimeError("six-container wave returned invalid results")
        if len({item["execution"]["container_name"] for item in results}) != 6:
            raise RuntimeError("container identities were not unique")
        print(f"PASS: six isolated containers completed concurrently in {elapsed:.2f}s")
