...
class CertificationRunnerTest(unittest.TestCase):
    ...
    def test_invalid_or_cyclic_plan_is_rejected_before_execution(self) -> None:
        with tempfile.TemporaryDirectory() as raw:
            ...
            self.assertEqual(completed.returncode, 2)
            self.assertIn("phase values are invalid", completed.stderr)
            self.assertFalse((root / "result.json").exists())
    ...
    def test_exact_phase_evidence_reuse_and_input_invalidators(self) -> None:
        with tempfile.TemporaryDirectory() as raw:
            ...
            invalidators = (
                {"factory_sha": "c" * 40},
                {"factory_sha": "c" * 40, "product_tree": "d" * 40},
                {
                    "factory_sha": "c" * 40,
                    "product_tree": "d" * 40,
            ...
            completed, result = self.run_plan(
                ...
                product_tree="d" * 40, network_reviewed=True,
            )
            self.assertEqual(completed.returncode, 0, completed.stderr)
            self.assertFalse(result["phases"][0]["cache_hit"])
            ...
            self.assertEqual(completed.returncode, 2)
            self.assertEqual(result["failure"]["reason_code"], "runtime_identity_mismatch")
            self.assertEqual(counter.read_text(), "5")
