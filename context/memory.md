...
- Dynamic CI selection is fail-closed and evidence-gated: six audited leaf components are active for local `--changed`, while required PR CI still shadows recommendations by running the full suite. Main, macOS compatibility, ambiguous diffs, and shared surfaces remain full.
...
Required GitHub CI remains a full shadow comparison. Any reproducible miss
demotes that component to shadow and resets its evidence.
