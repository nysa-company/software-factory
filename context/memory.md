...
- Provider-free ticket readiness rejects tracked static SHA-256 guards over any Builder or Test-author-owned path unless the exact protected test is both a declared conflict and a Fixture-Seam; semantic and dynamic source guards remain fail-closed Planner/Spec-linter work.
...
regular tracked tests for static `createHash('sha256')` plus `readFileSync`
guards and rejects any owned path unless that test is an exact declared
conflict and Fixture-Seam. Static-template and case variants remain covered,
tracked symlinks fail closed, and semantic or dynamic guards remain role work.
