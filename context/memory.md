...
- Factory development may use the macOS-only disposable lane against an exact clean commit, a synthetic product, and a local Git remote. Mock mode is network-denied and must finish below 15 minutes; real Cursor uses the authenticated CLI session with a one-use executable/session-bound approval. Only Cursor subprocesses receive the session home; Seatbelt permits exact reads of the two session files and credential services while denying normal-profile writes. Cursor's hardcoded temporary root is redirected into the lane for each sandbox invocation. Lane artifacts have no release receipts or activation records, and the production release contract is unchanged.
