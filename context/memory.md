...
- The development product scheduler retries one exact pinned-route authentication or model-availability miss after ten seconds only when no task was submitted. Every identity, version, contract, and post-submission failure remains terminal for that attempt.
- The disposable lane also has a four-ticket mock-concurrency mode. It reuses the Contract 1.6 coordinator, runtime, and executor with a lane-local fake container transport, proves provider overlap and reservation drain, then completes four synthetic role lifecycles; every runtime input must resolve beneath the validated owner-only lane root.
- Fresh isolated product proofs still require four tickets; a seeded retry may select one to four unfinished tickets from an owner-only bundle. Its accounting manifest binds the exact bundle and base, carries the full historical reservation map, and reduces both selected-ticket and aggregate lane budgets. V2 retains $100/$500 defaults; operator-authored v3 permits only the explicitly approved $200/$700 development ceilings and consumes one nonce on one fixed UTC day before lane creation. Canonical product sources, duplicates, seeded symlinks/submodules, and partial lease-claim residue fail closed.
- Development product export projects only the latest successful Reviewer's non-`factory/` tree changes and rejects later product drift. Exact bundles retain retry/audit history but are never applied to canonical product branches, preventing lane controls, route pins, sibling tickets, and unreviewed Narrator metadata from escaping.
- Contract 1.7 adds owner-activated subscription-CLI concurrency through the existing transactional coordinator. Activation v2 binds the exact allowed CLI route tuple and canonical provider-policy digest; Contract 1.6 accepts only API activation v1, and missing or invalid activation remains serialized.
