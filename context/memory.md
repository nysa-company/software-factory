...
- Successor qualification can preserve an exact source-passport checkpoint only through the existing protected in-flight release authorization; migration fast-forwards it without deleting or rewriting ticket work.
...
source-passport heads when a newer successor cannot replay their historical
role validation. Preparation still proves source HMAC, lineage, accounting,
commit/tree/ticket/route identity, and exact cohort membership. Route migration
may CAS-fast-forward an ancestor remote through that checkpoint, but never
deletes, rewinds, or rewrites its ticket history.
