...
- Protected in-flight release authorization accepts exact old-kit ticket heads from Ready through Approved; migration changes kit and route provenance without advancing ticket state.
...
- Factory development may use the macOS-only disposable lane against an exact clean commit, a synthetic product, and a local Git remote. Mock mode is network-denied and must finish below 15 minutes; real Cursor uses an owner-only browser-authenticated session created inside the lane with API keys, the normal Cursor profile, and Keychain access rejected and a one-use executable/session-bound approval. Lane artifacts have no release receipts or activation records, and the production release contract is unchanged.
...
lane-local browser-authenticated CLI session and a one-use approval bound to
the exact executable, owner-only session files, and lane inputs. The normal
Cursor profile and Keychain remain inaccessible. Development artifacts cannot be activated, and protected-main CI,
