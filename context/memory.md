...
- Qualification preparation serializes per project and resumes only a fresh,
  ...
  existing idempotent materializer runs again. If protected main advances after
  the Ready branch push, retry and dispatch require that canonical branch's
  audit digest to match the authenticated local receipt before using the
  existing reset/materialization path. Every other controller artifact still
  refuses. The signed
...
## 2026-08-07 — Decision 317: Pre-provider successor handoff moves exact cells

Category: Reliability

A fresh isolated successor may preserve a predecessor's unconsumed Planner
receipt only when the prior worker-error occurred before preflight, passport,
provider run, terminal, or publication. The helper authenticates both sealed
lanes, the identical cohort, exact protected reset heads, source claims,
released or exactly releasable leases, routes, remote heads, and origins while
holding controller plus admission/launch/lease boundaries. It quarantines the
old claims and uses only `git worktree move`. A digest journal under the durable
...
## 2026-08-11 — Decision 394: Real-Hermes canaries derive sequencing from the candidate

Category: Reliability

The credential-free real-Hermes release canary is generated by one repository
command after aggregate validation of its exact SHA/tree, short isolated root,
descriptor policy, capacity, disabled Linear state, and test-only provider-pin
scope. Its real profile hook delegates sequencing to the candidate's sealed
launcher: Contract 1.8 issues and consumes the exact state-machine transition
receipt, while supported older contracts retain `next-stage`. Production
profiles, credentials, provider routes, preview enforcement, and machine-global
provider-pin ownership remain outside the canary root and unchanged.
Partial runtime is resumable only behind an owner-only attempt marker bound to
...
## 2026-08-16 — Decision 464: Historical resume survives receipt migration

Category: Reliability

After exact successor migration resolves the prior-kit transition warning, the
sealed isolated driver may still enter reconciliation for one historical
`resume_receipt_mismatch` only when the current blocked claim binds the same
selected ticket and receipt. The controller remains the repair authority;
production, takeover, stale claims, and every other Doctor warning stay closed.

## 2026-08-16 — Decision 465: Historical resume is digest-bound, not clock-bound

Category: Reliability

During successor receipt migration, the prior-transition warning may be
observed after the durable historical resume mismatch. The isolated driver
...
through the current passport's signed migration lineage. Adjacent passport
...
## 2026-08-16 — Decision 472: Approval leaves the ticket head unchanged

Category: Reliability

`operator approve` projects only its owner-local one-use receipt and ignored
...
## 2026-08-19 — Decision 506: Qualification stops at a Claude spend limit

Category: Reliability

Q39 proved that Claude can pass readiness and later return an individual
spend-limit 429 after a submitted role has changed its cell. The release now
types only that bounded Claude result shape, retains its terminal accounting,
and latches qualification before another role launch. A dirty failed cell is
...
720-second ceiling. Publication-lease waiters recheck only while a sibling
...
## 2026-08-20 — Decision 520: Internet loss resumes exact post-provider work
...
as safe transport replay. Authentication, remote drift, malformed evidence, and
...
## 2026-08-21 — Decision 535: Operator preflight mirrors Ready ownership admission

Category: Reliability

Q67 preparation safely refused T-626 because protected Ready T-605 already
owned the same Builder path, but the earlier read-only operator preflight
reported no conflict because it compared only selected siblings. The report
...
## 2026-08-21 — Decision 547: Prepared Ready branches survive protected-main churn

Category: Reliability

If qualification preparation pushes a canonical Ready branch and a later
readiness check fails, a protected-main advance no longer strands the retry.
The owner-only receipt authenticates only the exact branch audit digest;
preparation and dispatch revalidate that canonical lineage before reusing the
existing exact-head reset/materialization path. Branch or receipt tamper still
...
## 2026-08-22 — Decision 563: Qualification readiness authenticates historical terminals

Category: Reliability

The shared ticket-readiness boundary validates every selected Done ticket with
the protected-terminal reader. Authenticated terminal evidence may retain its
...
## 2026-08-24 — Decision 595: Provider runtime cannot consume closeout polling

Category: Reliability

The sealed qualification finisher excludes Doctor and work before the first
...
wall-clock minutes or 120 polls. The existing finite progress bound caps those epochs, so
replay remains bounded without forcing a fresh Doctor merely because the total
pipeline ran longer than ten minutes.

## 2026-08-24 — Decision 596: Singleton publication waits retain one bounded controller

Category: Reliability

After the sealed isolated finisher first observes an authenticated closeout,
PR-gate, protected-merge, or publication-lease wait, it passes only the
decreasing remainder of that external epoch to reconciliation. The controller
retains the exact waiter and lease between polls in independent
maximum-30-second lock slices, then parks and releases on slice or epoch expiry.
...
## 2026-08-25 — Decision 603: Qualification claims follow the authenticated ledger

Category: Reliability

Doctor, controller reconciliation, upgrade/drain validation, cancellation, and
successor recovery resolve `.active-runs` beside the authenticated runtime
ledger, not a product-local approximation. A provider-only pre-GO recovery may
bind one exact dead claim whose owner PID matches that attempt, but only the
approved outer recovery under the controller/admission capability boundary can
...
## 2026-08-25 — Decision 610: Superseded fallback previews name the next check

Category: Reliability

A direct fallback preview for an older authenticated run remains fail-closed
...
## 2026-08-25 — Decision 612: Runtime validation precedes Planner command policy

Category: Reliability

Native CLI login shells validate Node and npm through the certified runtime
...
## 2026-08-26 — Decision 615: Dirty terminal recovery precedes passport export

Category: Reliability

A submitted failed Planner, Spec-linter, Test-author, Builder, or Narrator may
export its exact-once failed-role passport only after the controller preserves
policy-valid committed and dirty output under an immutable run ref and restores
the authenticated input tree. Replay accepts only the bound input, output, or
...
## 2026-08-27 — Decision 627: Role-control epochs survive successor activation

Category: Reliability

Qualification derives its immutable role-control epoch from the oldest record
in the existing authenticated activation-receipt chain. The current product
