...
- Factory development may use the macOS-only disposable lane against an exact clean commit, a synthetic product, and a local Git remote. Mock mode is network-denied and must finish below 15 minutes; real Cursor requires a dedicated API key and a one-use executable-bound approval. Because Cursor's `status` and `models` subcommands inspect only browser-login state, API-key mode validates credentials and model identity on the first model-bound call; failure cannot advance the lifecycle. Cursor's hardcoded temporary root is redirected into the lane for each sandbox invocation. Lane artifacts have no release receipts or activation records, and the production release contract is unchanged.
