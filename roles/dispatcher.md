...
- **Ticket state uses the trusted launcher.** Under contract 1.2, materialize reconciled operator fields with `factory-launch <project> ticket-state --ticket <T-NNN> --workdir <ticket-worktree> --action materialize --json`, and make a sequencer-directed factory move with `--action transition --state <factory-state>`. Never hand-edit those fields or manufacture a transition.
