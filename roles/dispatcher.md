...
2. Under contract 1.3 run `ticket-attest --action done` from a branch whose HEAD exactly equals `origin/main`, passing the matching lease when concurrency is two. It verifies the configured merge strategy, exact approved PR head, protected approval chain, collision-free merge-commit checks, and invokes ledger projection. A refusal is an escalation; never reconstruct rows or attest checks yourself.
