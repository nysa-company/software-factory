...
- **Bootstrap, pin, resolve, then preflight.** For a fresh ticket, create the exact clean `<TICKET_BRANCH_PREFIX><T-NNN>` linked worktree from current protected `origin/main`, then run trusted `ticket-state --action materialize`; that exact-SHA push creates and verifies the remote ticket ref. Next invoke `~/.factory/bin/factory-launch <project> models pin --ticket <T-NNN> --workdir <ticket-worktree> --json`. It must commit and push the Kit-SHA and exact six-role route plan together. After pinning, resolve `next-stage`, then preflight its exact role; Contract 1.5 rejects a roleless preflight. Any error is an escalation — do not launch, retry, or work around it.
...
- **The sequencer picks the stage, not you.** Before every launch, run `~/.factory/bin/factory-launch <project> next-stage --ticket <T-NNN> --workdir <ticket-worktree> --json` and obey its `action` (`RUN`, `FIX`, `AWAIT-OPERATOR`, `AWAIT-MERGE`, `COMPLETE`, `ESCALATE`, or `REFUSE`). Under Contract 1.7, immediately reconcile a successful Reviewer with trusted `ticket-state --action reviewer-reconcile`; it binds the durable output, records the exact verdict and repair owner, and moves a rejection from Review to Building in one commit. Never infer repair ownership or write Reviewer evidence yourself. Older contracts retain their manual ambiguous-fix behavior. After two Spec-linter FAILs, and before every later round, stop at the provider-free `AWAIT-OPERATOR`; only explicit operator instruction permits the exact `OPERATOR AUTHORIZATION: spec-linter round N` ticket line named by the sequencer. Never infer or pre-write it. Reviewer has no semantic-round authorization gate.
- **Prepare the ticket PR before review.** Under Contract 1.6, when the sequencer returns `RUN reviewer` after Builder, call trusted `ticket-pr` for the exact worktree and matching renewed lease before launching Reviewer. It creates or reuses one exact open PR and cannot approve or merge. A refusal is an escalation.
...
- **Never merge, never approve.** The operator's receipt-bound approval (`operator approve`) is the only business approval. Contract 1.3's trusted helper may request protected auto-merge; you never approve, force, bypass, or directly merge.
...
- **Budget-only review loop.** Route every REQUEST CHANGES to its exact
  `FIX-OWNER`; admission stops when the ticket budget is exhausted, not at a
  semantic round count.
- **Obey the public concurrency limit.** Contract 1.0 and newer contracts with a reported limit of 1 remain one-ticket-at-a-time. Contracts 1.2 through 1.4 inherit 1.1 lease behavior: with a reported limit from 2 through 4, claim no more than that many distinct tickets, renew and pass each matching opaque lease before sequencing, launch, or any contract-1.3+ ticket attestation, and release it only at sequencer `COMPLETE` or Blocked-Escalated. The product-wide provider lock still serializes model-provider intervals; multiple leases do not authorize simultaneous provider calls. Never log or persist the lease ID elsewhere. Capacity refusal, a stale or mismatched lease, or unsafe lease state is an escalation, never a reassignment.
...
- **Operator field ownership is binding.** Never manufacture priority, initiative, Ready, approval, or an unblock in the ticket file. Reconciliation reads operator-issued receipts into the ignored operator map; the trusted `ticket-state` path materializes them. You may move factory-owned role stages and escalate only through that same launcher command.
...
2. Under contract 1.3 run `ticket-attest --action done` from a branch whose HEAD initially equals `origin/main`, passing the matching lease when concurrency is greater than one. It verifies the configured merge strategy, exact approved PR head, protected approval chain, and collision-free merge-commit checks; projects accounting once; then creates or reuses the exact factory-owned closeout PR and requests protected auto-merge. Retry the same command only after its documented push/PR/auto-merge network-boundary failure; it must reuse the exact attested closeout commit. Every other refusal is an escalation.
...
- v17: Contract 1.6 expands the single coupled worktree/provider capacity to six and permits only exact owner-activated API routes to bypass the legacy provider lock.
