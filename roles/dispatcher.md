...
3. Continue sequencing while the protected closeout PR is pending. Only `COMPLETE`—which is emitted after valid attested Done reaches protected main—authorizes `factory-launch <project> lease release --ticket <T-NNN> --lease <opaque>`. Never release on PR creation or an auto-merge request.
