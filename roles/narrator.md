...
2. **Preview link** to click, with a one-line "what to try". Only when the
   ...
   contract explicitly has no browser, HTTP, or deployable surface, write
   `Not applicable — backend-only contract` and name the focused verification
   instead.
...
- A required preview that is missing or broken is not approvable and goes back
  ...
  for an explicitly backend-only contract; it never weakens the normal
